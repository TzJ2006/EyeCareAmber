...
On the built-in display Amber estimates the resulting backlight in nits. That closes a loop relative attenuation alone cannot: the same ×0.75 factor is 119 nits at a 400-nit backlight and 8.9 nits at 30 nits, and the second one is below every published comfort floor. Once the screen is already dark enough, Amber stops dimming further — it never makes the display harder to read than leaving it off.

**The nits the IORegistry reports are not usable.** On a Mac16,7 running macOS 26.5.2 that key never updates: with the sensor covered, macOS moved its own brightness slider within one second and the screen visibly changed, while `BrightnessMilliNits` stayed byte-identical for seven hours, alongside frozen `brightness`, `rawBrightness` and `BrightnessMicroAmps`, and a `CurrentNits` pinned at 0. The `backlight report / brightness report` IOReport channels return the same dead values, and no other node in the IORegistry publishes nits. That machine reports `new-backlight-architecture = Yes`, so the older keys appear to be vestigial. A frozen value that looks plausible is worse than no value — v1.1.0 shipped exactly that and displayed it as a measurement.

So the figure is **modeled, not measured**, and rebuilt from two live `DisplayServices` scales instead:

```
full scale = panel minimum luminance / minimum usable linear brightness
nits       = linear brightness × full scale
```

Three things support treating linear brightness as proportional to luminance: it reads exactly 1.0000 at maximum slider position (with auto-brightness off — leave it on and it caps around 0.667), it moves monotonically with the slider across the whole travel, and the full scale that falls out — 586 nits on the machine above — lands within 3% of that panel's rated 600-nit SDR maximum. The derivation is self-consistent too: "the minimum usable linear brightness" and "the panel's minimum luminance" describe the same physical point. The panel minimum comes from `BrightnessMilliNits.min`, which is structural metadata rather than the value that freezes.

It has never been checked against a photometer, so the UI says "about N nits, a modeled estimate", and where the model cannot be built — external displays, Intel Macs, a renamed symbol — Amber treats the display as unreadable and runs the time curve on relative attenuation alone.
...
The night pair is chosen to satisfy both endpoints at once. Luminance is fixed first: about 30% relative output lands near 36 cd/m² at a typical dim-room backlight, matching the low-stimulus arm of Li et al. (2026) that improved DLMO, cortisol, subjective sleep, visual fatigue **and** cognition together — the one measured luminance where both endpoints move the right way. A screen that is too dim has its own cost, but the study usually cited for that does not measure what it gets quoted as measuring: in Yu & Akita (2019) the 9 and 25 cd/m² figures are the **room's** luminance, not the screen's — reading a tablet in the darker room brought on physical, psychological and visual fatigue, in the less dark room only visual. That is evidence about the screen-to-room ratio, not about an absolute floor for the screen.
...
ISO 9241-303 suggests 100–150 cd/m² for screens at 500 lx horizontal illuminance. Under low light the comfortable band is far lower, though the literature is not tight about where: Na & Suk (2015) measured 10 cd/m² for a first glance and 40 for continuous viewing, Zhou et al. (2021) 20.63 to 36.20 at 0 lx across three endpoints, Lin et al. (2022) 63.9 at 1 lx — a sixfold spread across studies. Amber treats 20 cd/m² as a floor rather than a target, and takes it from the bottom of that spread rather than from any single study. That spread is only about near-dark rooms; once the room is lit the same literature disagrees by a factor of three — Kim et al. (2017) put the lower comfort limit at 113 cd/m² for 50 lx, where Zhou et al. (2021) measured optima of 34.5 and 41.4 cd/m² at the same illuminance. There is no ambient-independent floor. Keeping noon-level screen brightness after dark is a major source of evening discomfort — hence the dusk transition.
...
- Absolute nits are **modeled, never measured**, and only on Apple Silicon built-in displays, from two undocumented `DisplayServices` symbols plus one IORegistry metadata field, any of which Apple may rename. The model has not been checked against a photometer. On external displays, on Intel Macs, or if any input goes missing, Amber falls back to relative attenuation and the comfort floor cannot be enforced.
...
This project is not a medical device and does not give medical advice. Parameters are **engineering starting points** from public literature, not clinical thresholds. Individual sensitivity to nighttime light varies more than 50-fold: Phillips et al. (2019) found half-maximal melatonin suppression anywhere from 6 to 350 lux across 55 people. For sleep or eye conditions, see a professional.
